...
$(MPOBJ): obj/%_mp.o: src/%.cpp
	$(GCC) $(OPT) -fopenmp -gdwarf-3 -DMP -c -o $@ $< $(CONFIGINC) $(FFTW3INC) -DCPUCOMP=$(CPUCOMP) -DGPUCOMP=$(GPUCOMP) -DHOSTNAME=$(HOSTNAME) -DGIT_SHA1=$(GITINFO) -DGITDIRTY=$(GITDIRTY)

$(OBJECTS): obj/%.o: src/%.cpp
	$(GCC) $(OPT) -gdwarf-3 -c -o $@ $< $(CONFIGINC) $(FFTW3INC) -DCPUCOMP=$(CPUCOMP) -DGPUCOMP=$(GPUCOMP) -DHOSTNAME=$(HOSTNAME) -DGIT_SHA1=$(GITINFO) -DGITDIRTY=$(GITDIRTY)

ASDmp: $(MPOBJ)
	$(GCC) -fopenmp -gdwarf-3 -DMP $(OPT) -o $@ $^ $(CONFIGLIB) $(FFTW3LIB) $(CULIBS)

ASDcu: $(CUDAOBJ) $(NVCCOBJ)
	$(NVCC) -DCUDA $(OPT) -o $@ $^ $(CONFIGLIB) $(FFTW3LIB) $(CULIBS)

ASD: $(OBJECTS)
	$(GCC) $(OPT) -gdwarf-3 -o $@ $^ $(CONFIGLIB) $(FFTW3LIB) 
